...
mod tests {
    ...
    fn test_basic_cache_operations() {
        ...
        cache.put("key1".to_string(), b"value1".to_vec(), 0).unwrap();
    ...
    fn test_lru_eviction() {
        let cache = LockFreeCache::new(100); // Small cache
        
        // Fill cache
        cache.put("key1".to_string(), vec![0u8; 40], 0).unwrap();
        cache.put("key2".to_string(), vec![0u8; 40], 0).unwrap();
        
        // Access key1 to make it more recent
        cache.get("key1");
        
        // Insert new entry that requires eviction
        cache.put("key3".to_string(), vec![0u8; 40], 0).unwrap();
    ...
    fn test_ttl_expiration() {
        ...
        cache.put("key1".to_string(), b"value1".to_vec(), 1).unwrap();
        assert!(cache.get("key1").is_some());
